...
class validate_quiz_keys extends external_api {
    ...
    /**
     * Validate a proctor config key or browser exam key.
     *
     * @param string $cmid Course module ID.
     * @param string $url URL of the page on which the proctor JS API generated the keys.
     * @param string|null $configkey A proctor config key hash. Includes URL in the hash.
     * @param string|null $browserexamkey A proctor browser exam key hash. Includes the URL in the hash.
     * @return array
     */
    public static function execute(string $cmid, string $url, ?string $configkey = null, ?string $browserexamkey = null): array {
        ...
        // At least one proctor key must be provided.
        if (empty($configkey) && empty($browserexamkey)) {
            throw new invalid_parameter_exception(get_string('error:ws:nokeyprovided', 'quizaccess_proctor'));
        }

        // Check quiz exists corresponding to cmid.
        if (($quizid = self::get_quiz_id($cmid)) === 0) {
            throw new invalid_parameter_exception(get_string('error:ws:quiznotexists', 'quizaccess_proctor', $cmid));
        }
